...
from cog.core import Table, Record, Store, TableMeta
...
class TestStoreBufferedWriteCoherency(unittest.TestCase):
    """The read mmap must reflect buffered writes that the OS cannot yet see
    (batch mode / flush_interval > 1), even with the record cache disabled."""

    @classmethod
    def setUpClass(cls):
        cls.db_path = "/tmp/TestStoreBufferedWriteCoherency"
        if os.path.exists(cls.db_path):
            shutil.rmtree(cls.db_path)
        os.makedirs(cls.db_path + "/test_ns/")
        config.CUSTOM_COG_DB_PATH = cls.db_path
    ...
    @staticmethod
    def _store(name, **kwargs):
        # caching_enabled=False forces every read onto the mmap/disk path, so
        # nothing is masked by the record cache.
        meta = TableMeta(name, "test_ns", "tid0", False)
        return Store(meta, config, logging.getLogger(), caching_enabled=False,
                     **kwargs)

    def test_inplace_update_visible_to_mmap_in_batch(self):
        store = self._store("buf_coherency")

        # Write + flush a record so it lands in the mmap with key_link == -1.
        pos = store.save(Record(key="n1", value="v1", value_type="s"))
        first = store.read(pos)
        self.assertEqual(first.key_link, Record.RECORD_LINK_NULL)

        # In batch mode the in-place link update stays in the buffered writer.
        store.begin_batch()
        store.update_record_link_inplace(pos, 4242)

        # Read must reflect the buffered update, not the stale mmap bytes.
        updated = store.read(pos)
        self.assertEqual(updated.key_link, 4242,
                         "read returned stale key_link — mmap not coherent with "
                         "buffered write")

        store.end_batch()
        self.assertEqual(store.read(pos).key_link, 4242)
        store.close()

    def test_appended_record_visible_before_flush(self):
        # flush_interval high enough that batch-style appends stay buffered.
        store = self._store("buf_append", flush_interval=1000)
        positions = []
        for i in range(50):
            positions.append(
                (store.save(Record(key="k%d" % i, value="val%d" % i,
                                   value_type="s")), i))
        # No explicit flush; reads must still see the buffered records.
        for pos, i in positions:
            rec = store.read(pos)
            self.assertIsNotNone(rec, "buffered append unreadable at pos=%d" % pos)
            self.assertEqual(rec.value, "val%d" % i)
        store.close()
