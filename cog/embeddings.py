...
class EmbeddingMixin:
    ...
    def put_embedding(self, word, embedding):
        ...
        if self._cloud:
            self._cloud_client.mutate_put_embedding(word, embedding)
            return
    ...
    def get_embedding(self, word):
        ...
        assert isinstance(word, str), "word must be a string"
        if self._cloud:
            result = self._cloud_client.query_get_embedding(word)
    ...
    def delete_embedding(self, word):
        ...
        assert isinstance(word, str), "word must be a string"
        if self._cloud:
            self._cloud_client.mutate_delete_embedding(word)
            return
    ...
    def put_embeddings_batch(self, word_embedding_pairs):
        ...
        """
        if self._cloud:
            batch = [{"word": w, "embedding": e} for w, e in word_embedding_pairs]
    ...
    def k_nearest(self, word, k=10):
        ...
        """
        if self._cloud:
            return self._cloud_append("k_nearest", word=word, k=k)
    ...
    """
        if self._cloud:
            return self._cloud_append("sim", word=word, operator=operator,
        ...
        filtered_vertices = []
        for v in self.last_visited_vertices:
            similarity = self._cosine_similarity(word, v.id)
            if not similarity:
    ...
    def vectorize(self, words=None, provider="cogdb", batch_size=100, **kwargs):
        ...
        if self._cloud:
            ...
            if isinstance(w, str):
                w = [w]
            return self._cloud_client.mutate_vectorize(w, provider, batch_size)
