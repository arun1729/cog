...
class EmbeddingMixin:
    ...
    def put_embeddings_batch(self, text_embedding_pairs):
        ...
        Example:
            g.put_embeddings_batch([
                ("machine learning is a subfield of AI", [0.1, 0.2, ...]),
                ("neural networks learn from data", [0.3, 0.4, ...]),
            ])
        """
        if self._cloud:
    ...
    def vectorize(self, texts=None, provider="cogdb", batch_size=100, **kwargs):
        ...
        Can embed all graph nodes, a single string, or a list of strings.
        Supports arbitrary text input — words, phrases, or sentences — as accepted
        by the underlying embedding model (e.g. BGE-small, OpenAI text-embedding-3-small).
        ...
        # Determine which texts to embed
